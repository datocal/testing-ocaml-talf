 ...
 * af_of_er : er -> af
 *
 * Función que dada una expresión regular devuelve el autómata finito        
 * correspondiente.                                                          
 *
 *****************************************************************************)
...
let af_of_er e = 
   let
      simbolos = simbolo_of_er e
   in
      let rec aux n = function
           Vacio -> 
              let
                 m = nuevo_estado n simbolos
              in
                 let
                    s = string_of_int m
                 in
                    (Af (Conjunto [Estado s],
                         Conjunto [], 
                         Estado s, 
                         Conjunto [], 
                         Conjunto []), 
         ...
         | Constante (Terminal s) ->
              let
                 m1 = nuevo_estado n simbolos
              in
                 let
                    m2 = nuevo_estado (m1+1) simbolos
                 in
                    let
                       s1 = string_of_int m1
                    and
                       s2 = string_of_int m2
                    in
                       (Af (Conjunto [Estado s1; Estado s2],
                            Conjunto [Terminal s],
                            Estado s1,
                            Conjunto [Arco_af (Estado s1, Estado s2, Terminal s)],
                            Conjunto [Estado s2]),
                        m2+1)
   
         | Constante _ ->
              raise (Failure "af_of_er: la expresion regular contiene no terminales")
         ...
         | Union (e1,e2) ->
              ...
              in
                 let
                    s = string_of_int m
                 and
                    (Af (q1, t1, i1, d1, f1), m1) = aux (m+1) e1
                 in
                    let
                       (Af (q2, t2, i2, d2, f2), m2) = aux m1 e2
                    in
                       (Af (union (Conjunto [Estado s]) (union q1 q2),
                            union t1 t2,
                            Estado s,
                            union (Conjunto [Arco_af (Estado s, i1, Terminal "");
                                             Arco_af (Estado s, i2, Terminal "")])
                                  (union d1 d2),
                            union f1 f2),
                        m2)
              ...
              in
                 ...
                 in
                    (Af (union q1 q2,
                         ...
                         (union d1 
                                (union (Conjunto 
                                   (map (function q -> Arco_af (q, i2, Terminal "")) 
                                      (list_of_conjunto f1)))
                                       d2)),
                         f2),
                     m2)
         ...
         | Repeticion e ->
              ...
              in
                 let
                    s = string_of_int m
                 and
                    (Af (q, t, i, d, f), m1) = aux (m+1) e
                 in
                    (Af (union (Conjunto [Estado s]) q,
                         t,
                         Estado s,
                         union (Conjunto [Arco_af (Estado s, i, Terminal "")])
                               (union d
                                      (Conjunto
                                         (map (function x -> Arco_af (x, Estado s, Terminal ""))
                                         (list_of_conjunto f)))),
                         Conjunto [Estado s]),
                     m1)
